...
async function sendRunlogTransaction({
  ...
  await linkToken
    .transfer(runLog.address, linkAmount, {
      gasLimit: 100000,
    })
    .catch(() => {
      throw Error('Error transferring link to RunLog')
    })
...
async function makeRunlogRequest(runLog: RunLog, job: any) {
  await runLog
    .request(ethers.utils.toUtf8Bytes(job.data.id), {
      gasLimit: 2000000,
    })
    .catch((e: any) => {
      throw Error(`Error making RunLog request: ${e}`)
    })
